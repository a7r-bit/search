import { Controller, Get, Post, Body, Patch, Param, Delete, UseInterceptors, UploadedFile, ParseFilePipe, MaxFileSizeValidator, BadRequestException, Query } from '@nestjs/common';
...
export class DocumentVersionController {
  ...
  async findByDocument(
    ...
    return await this.documentVersionService.findByDocumentId(documentId, filterDto, sort);
  }
  ...
  @ApiOperation({
    ...
    description: `Создание новой версии документа, передавая documentId — ID документа и файл. 
  Поддерживаемые типы файлов: .doc, .docx, .pdf. Максимальный размер — 5MB.`,
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    description: 'Форма для загрузки файла и указания ID документа',
    schema: {
      type: 'object',
      properties: {
        documentId: {
  ...
  @ApiCreatedResponse({ description: "Успешное создание версии документа", type: DocumentVersionDto })
  @ApiResponse({
    status: 404, description: "Запись о документе не найденаы", example:
    {
  ...
  @ApiResponse({
    status: 400, description: "Неподдерживаемый формат файла или превышение размера", example: {
  ...
  async create(@Body() createDocumentVersionDto: CreateDocumentVersionDto, @UploadedFile(
    new ParseFilePipe({
      validators: [
  ...
  @Patch(':id')
  @ApiOperation({ summary: `Обновление версии документа`, description: `Обновление версии или названия файла переданного в параметраъ`, })
  @ApiResponse({ description: "Успешное обновление версии документа", type: DocumentVersionDto })
  async updateDocumentVersion(@Param('id', new CustomParseUUIDPipe()) documentVersionId: string, @Body() update: UpdateDocumentVersionDto): Promise<DocumentVersionDto> {
  ...
  @ApiResponse({
    status: 200, description: "Версия документа успешно удалена", type: DocumentVersionDto
  })
  @ApiResponse({
    status: 404, description: "Запись о документе не найдена",
  })
